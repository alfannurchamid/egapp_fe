<script>
  ...
  const divisi = [
    { nama_divisi: "le coffee" },
    { nama_divisi: "le skin" },
    { nama_divisi: "the kotenang" },
    { nama_divisi: "agro bisnis" },
    { nama_divisi: "BPNT" },
    { nama_divisi: "LPG" },
    { nama_divisi: "semen" },
    { nama_divisi: "konstruksi" },
  ];
</script>

<div
  class=" relative w-full h-20 max-w-screen-md overflow-x-scroll mt-24 flex items-center"
>
  <div
    style=" width: auto;"
    class="flex absolute w-full h-16 items-center px-2"
  >
    {#each divisi as div}
      <div class="flex text-gray-700 items-center min-w-24 flex-col mx-2.5">
        <div class="  h-14 w-14 rounded-full bg-gray-400"></div>
        <ul>{div.nama_divisi}</ul>
      </div>
    {/each}
  </div>
</div>

<div class=" w-full flex mt-10 px-8">
  ...
  <input
    type="text"
    class="bg-gray-100 outline-none rounded-r-lg h-10 w-full px-3 placeholder:text-lg placeholder:font-light font-light text-lg"
