<script>
  ...
  /*  const apiBaseUrl = import.meta.env.VITE_API_BASE_URL v1/auth/login;
  const endpoint = `${apiBaseUrl}v1/auth/login`; */
  ...
  onMount(async () => {
    ...
    if (await Falidate()) {
      ...
      await Portal($user.access, $user.id_karyawan, $user.divisi);
    }
  });
