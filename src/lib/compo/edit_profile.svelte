...
<div
  ...
  class=" w-screen fixed top-0 z-50 bg-gray-100 p-5 min-h-screen flex flex-col py-5"
>
  ...
  <div class=" w-full bg-white rounded-xl p-5 flex flex-col">
    <div class=" flex border-b">
      <div class=" flex flex-col items-center">
        <div
          id="photoP"
          class=" min-w-20 min-h-20 bg-slate-200 rounded-full"
        ></div>
        <label for="photoP" class="mt-2 text-green-600">Edit</label>
      </div>
      <p class=" text-gray-700 text-sm ml-5">
        Ubah nama panggilan anda dan tambahkan foto profil (opsional)
      </p>
    </div>
    <div class=" w-full border-b flex items-center">
      <input
        ...
        name="nama_panggilan"
        id="nama_panggilan"
        class="w-full h-10 focus:outline-none text-xl font-light"
      />
      {#if input_focus}
    ...
    <div class=" w-full border-b h-10 flex items-center">
      <label for="" class="w-full h-min text-gray-500 text-xl font-light"
        >{nama_lengkap}
      </label>
    </div>
  </div>
  ...
  <div class="w-full rounded-xl bg-white py-5 flex flex-col mt-10 p-5">
    ...
    <div class=" w-full border-b flex items-center">
      <input
        on:focus={() => (input_wa_focus = true)}
        on:blur={() => (input_wa_focus = false)}
        bind:value={no_wa}
        type="text"
        name="nowa"
        id="nama_panggilan"
  ...
  <button
    ...
    class="TbPasif text-gray-500 w-24 mt-10"
  >
    keluar</button
  >
</div>
