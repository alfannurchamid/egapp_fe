...
<div
  class=" rencanakerja_card p-1 px-2 min-h-12 my-2 w-full bg-white drop-shadow flex rounded-lg"
>
  ...
  <div class=" w-2/6 flex flex-col justify-start pl-2 border-l ml-1">
    <div class=" flex justify-around mb-1">
      <button
        on:click={() => {
          currentOpenElementEditId(rencana_kerja.id_renker);
        }}
      >
      ...
      <div
        class=" w-16 h-3 rounded bg-green-500 text-xxs text-center flex items-center justify-center text-white"
      >
        Tercapai
      </div>
    </div>
    ...
    <div name="progres" class=" mt-1 w-28 h-1 bg-slate-200">
      <div
        ...
        style="width: {rencana_kerja.progres}%;"
      ></div>
    </div>
  </div>
</div>
...
{#if $open_element_id == rencana_kerja.id_renker}
  <EditRencanaKerja {rencana_kerja}></EditRencanaKerja>
{/if}
