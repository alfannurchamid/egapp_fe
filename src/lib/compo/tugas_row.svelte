...
<div class=" flex w-full bg-slate-50 my-2 py-2 px-5 mt-{tugas.id} ">
  <div class=" items-center flex border-r pr-2">
    <div
      class=" h-8 w-8 rounded-full bg-slate-200 flex justify-center items-center"
    >
  ...
  <div class=" block w-full">
    <div class=" flex justify-between flex-row w-full mb-2">
      <button
        ...
        class=" text-xs ml-2">{tugas.judul}</button
      >
      <div class=" flex justify-between max-w-32">
        <button
          ...
          class=" "
          on:click={() => {
            currentOpenEditTugas(tugas.id_tugas);
          }}
        >
    ...
    <p class=" text-gray-600 text-xxs ml-2 text-end">
      start date : {tugas.deadline.substr(0, 10)}
    </p>
    <p class=" text-gray-600 text-xxs ml-2 text-end">
      deadline : {tugas.start_date.substr(0, 10)}
    </p>
  </div>
</div>
...
<div>     
  ...
  <EditTugas {tugas} {karyawans}></EditTugas>
{/if}
