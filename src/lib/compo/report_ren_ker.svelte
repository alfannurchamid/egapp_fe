...
>>>>>>> e3cd1e68cacc7cc5b78dca51d25603c6a2a46bdb
  ...
  let accessKey = "";
  let id_renker = $page.params.id_rk;
...
=======
  let values_to_update = { id_renker: renker.id_renker };
...
<<<<<<< HEAD
      const upload_file_report = async()=>{
            ...
            var newFile = new File([blob], renker.id_renker.toString()+'_renker_'+file.name, {type: file.type});
...
=======
  const upload_file_report = async () => {
    ...
    var newFile = new File(
      [blob],
      renker.id_renker.toString() + "_renker_" + file.name,
      { type: file.type }
    );
    if (newFile.size < 3000000) {
      data.append("file", newFile);
...
<<<<<<< HEAD
          ...
          <div class=" flex w-full p-2 mt-3 flex-col">
            ...
            <input
              ...
              max="100"
              min="0"
              bind:value={renker.progres}
              class="my-2"
            />
            <button
              on:click={() => {
                update_capaian("progres", renker.progres);
              }}
