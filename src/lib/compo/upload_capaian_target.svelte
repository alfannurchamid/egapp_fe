<script>
  // @ts-nocheck
    ...
    import { open_pop_up_capaian, currentOpenUpCapaian } from "$lib/stores/openPopTarget";
    ...
    const upload_file_report = async()=>{
          ...
          var newFile = new File([blob], target.id_target.toString()+'_target_'+file.name, {type: file.type});
      ...
      const download_file_report = async()=>{
          ...
          accessKey =  GetCookie('accesskey')
            const response = await fetch(
              "https://be.ekagroup.co/api/api/v1/divisi/report?file="+target.file_name+"&type=target",
          {
                  method: "GET",
                  headers: {
                      "Content-Type": "application/json",
        ...
        <div class=" flex ">  
          ...
          <div class="w-80 flex text-white bg-black rounded-tl-3xl rounded-b-3xl  p-5 bg-opacity-40  flex-col items-center  ">
              ...
              {:else if access_user == 4}
                    ...
                    {:else if status_report == 4}
                      <!-- aksi setelah report -->
                        <h3 class=" text-center mb-3">Manager telah mengunggah file report , silahkan review dari direksi</h3>
